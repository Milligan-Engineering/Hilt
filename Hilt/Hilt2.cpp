...
string classTwoFiles[MAX_STUDENTS] = { "astonishing", "astounding", "blindsiding", "dumbfounding", "eye-opening", "flabbergasting", "jarring", "jaw-dropping", "jolting", "shocking", "startling", "stunning", "stupefying", "surprising" };//Since I am currently just renaming the array elements without using the data contained in a meaningful way, I simply populated classTwofiles with random strings(the synonyms of amazing)
...
int main()
{
	...
	while (userInputSearch == 'y')
	{
		...
		arrayMatches = 0;
		for (int p = 0; p < (numberStudents-1); p++)
		{
			if (testStorage[p] != -1)
			{
